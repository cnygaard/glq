"""Phase 1.3 acceptance gate: per-group scale broadcast.

Kernel ``kernel_unified_attention_2d_e8_v1_3`` extends Phase 1.2 with
the per-group ``scale`` multiplication. Decode math:

    dec = (codebook[idx1] + codebook[idx2] / RS) * scale[group]

Where ``scale`` has one fp16 value per (token, head, group of 8). The
Hadamard inverse still isn't applied (Phase 1.4 closes that gap).
"""
...
def _quantize_to_paged(K, V, quantizer, *, num_blocks, block_size):
    ...
    def _pack_scale(scale_tensor):
        # ``scale`` is [..., G] -> reshape to [Tk, H_kv, G].
    ...
    k_idx1 = _pack_int16(qk["idx1"])
    k_idx2 = _pack_int16(qk["idx2"])
    v_idx1 = _pack_int16(qv["idx1"])
    v_idx2 = _pack_int16(qv["idx2"])
    k_scale = _pack_scale(qk["scale"])
    v_scale = _pack_scale(qv["scale"])
    return k_idx1, k_idx2, k_scale, v_idx1, v_idx2, v_scale


def _kernel_truth_decode_v1_3(idx1_p, idx2_p, scale_p, codebook, rs, *,
                              num_tokens, num_kv_heads, head_size):
    """Reproduce Phase 1.3 dequant: scale * (codebook[idx1] + codebook[idx2] / RS).

    No Hadamard yet.
    """
    nb, bs, H_kv, G = idx1_p.shape
    ...
    f1 = _flat_idx(idx1_p)
    f2 = _flat_idx(idx2_p)
    sc = scale_p.reshape(nb * bs, H_kv, G)[:num_tokens].to(torch.float32)
...
def _run_forked_v1_3(q, k_idx1, k_idx2, k_scale, v_idx1, v_idx2, v_scale,
                     codebook, resid_scale, *, Tk, softmax_scale):
    from glq_vllm.triton_unified_attention_e8 import (
        unified_attention_e8_v1_3,
    )
    Tq = q.shape[0]
...
def _run_case(*, Tq, Tk, head_size, block_size, seed, quantizer):
    ...
    K_truth = _kernel_truth_decode_v1_3(
        k_idx1, k_idx2, k_scale, codebook, rs,
        num_tokens=Tk, num_kv_heads=1, head_size=head_size,
    )
    V_truth = _kernel_truth_decode_v1_3(
        v_idx1, v_idx2, v_scale, codebook, rs,
        num_tokens=Tk, num_kv_heads=1, head_size=head_size,
    )
    ...
    out_fk = _run_forked_v1_3(
        q, k_idx1, k_idx2, k_scale, v_idx1, v_idx2, v_scale, codebook, rs,
...
def test_phase_1_3_decode_tiny(quantizer_stage2):
...
def test_phase_1_3_prefill(quantizer_stage2):
    _, _, max_abs, rmse = _run_case(
...
def test_phase_1_3_two_blocks(quantizer_stage2):
    _, _, max_abs, rmse = _run_case(
