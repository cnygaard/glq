...
def _ensure_registered():
    ...
    # 3INST (lookup-free V=1) fused trellis linear: same bracket + B-dispatch, NO tlut arg —
    # the kernel decodes arithmetically with zero dynamic smem. Separate op because the HYB
    # op's schema requires the tlut tensor.
    if hasattr(cuda, "glq_fused_linear_trellis_3inst_cuda"):
        _glq_lib.define(
            "fused_linear_trellis_3inst(Tensor x, Tensor sv, Tensor su, "
            "Tensor trellis_packed, "
            "Tensor blocks_n, Tensor blocks_m, Tensor blocks_n_meta, Tensor blocks_m_meta, "
            "float wscale, int in_features, int out_features, "
            "int n_pad, int m_pad) -> Tensor")
        _glq_lib.impl("fused_linear_trellis_3inst",
...
def _fused_linear_trellis_3inst_fake(x, sv, su, trellis_packed,
                                     blocks_n, blocks_m, blocks_n_meta, blocks_m_meta,
                                     wscale, in_features, out_features, n_pad, m_pad):
