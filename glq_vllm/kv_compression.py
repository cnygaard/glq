...
def _patched_unified_attention(*, q, k, v, **kwargs):
    ...
    v0.5 Phase 3 attempted to add an inline-dequant fast path via
    ``GLQ_KV_E8_INLINE_DEQUANT=1``; reverted in Phase 4a (commit
    landing after ``bc225d6``) because the inline kernel was 33 %
    slower at max_tokens=16384 — the codebook gather inside the
    attention tile loop costs more than the v0.3.5 workspace path it
    was meant to replace. The kernel itself (``unified_attention_e8_v2_1``
