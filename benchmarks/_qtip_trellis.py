"""QTIP trellis codebook (bitshift_codebook) — EXTRACTED from Cornell-RelaxML/qtip
`lib/codebook/bitshift.py` for a standalone codebook-level MSE experiment.

QTIP (NeurIPS 2024) is GPL-3.0, same license as GLQ. Only the pure-torch Viterbi
encode + LUT decode is kept; the CUDA decode kernel (BitshiftLinear) and the
`lib.*` imports it needs are dropped. Bitrate = K bits/weight.
Source: https://github.com/Cornell-RelaxML/qtip
"""
import itertools
import math
import os
